...
def main(**kwargs):
    ...
    exp22['dataset'] = {}
    exp22['sim'] = {}
    exp22['mode'] = 'benchmark'
    exp22['model'] = 'lin-encoder-nn-relu'
    exp22['exp_name'] = 'exp22'
    exp22['dataset_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'data', 'benchmark_stimuli', 'N8'))
    exp22['save_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'results'))
    
    ## Macrofeature parameters
    exp22['dataset']['size'] = 1000
    exp22['dataset']['i'] = None
    exp22['dataset']['k'] = None
    exp22['dataset']['l'] = None
    exp22['dataset']['m'] = None
    exp22['dataset']['s'] = None
    exp22['dataset']['s_list'] = None

    ## Category parameters
    ### In order, each element contains: k, d, pdi, pd
    exp22['dataset']['custom'] = None 
    
    # Simulation parameters
    exp22['sim']['layer_params'], exp22['sim']['sim_params'] = sim_config() 
    
    # Repetition parameter
    exp22['repeat'] = 0

    with open(save_fname, 'w') as f:
        json.dump(exp22, f, indent=3)
