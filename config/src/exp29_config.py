...
def main(**kwargs):
    ...
    exp29['dataset'] = {}
    exp29['sim'] = {}
    exp29['mode'] = 'benchmark'
    exp29['model'] = 'nn-sig'
    exp29['exp_name'] = 'exp29'
    exp29['dataset_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'data', 'benchmark_stimuli', 'N8'))
    exp29['save_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'results'))
    
    ## Macrofeature parameters
    exp29['dataset']['size'] = 1000
    exp29['dataset']['i'] = None
    exp29['dataset']['k'] = None
    exp29['dataset']['l'] = None
    exp29['dataset']['m'] = None
    exp29['dataset']['s'] = None
    exp29['dataset']['s_list'] = None

    ## Category parameters
    ### In order, each element contains: k, d, pdi, pd
    exp29['dataset']['custom'] = None 
    
    # Simulation parameters
    exp29['sim']['layer_params'], exp29['sim']['sim_params'] = sim_config() 
    
    with open(save_fname, 'w') as f:
        json.dump(exp29, f, indent=3)
