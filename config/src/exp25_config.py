...
def main(**kwargs):
    ...
    exp25['dataset'] = {}
    exp25['sim'] = {}
    exp25['mode'] = 'benchmark'
    exp25['model'] = 'lin-decoder-relu'
    exp25['exp_name'] = 'exp25'
    exp25['dataset_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'data', 'benchmark_stimuli', 'N8'))
    exp25['save_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'results'))
    
    ## Macrofeature parameters
    exp25['dataset']['size'] = 1000
    exp25['dataset']['i'] = None
    exp25['dataset']['k'] = None
    exp25['dataset']['l'] = None
    exp25['dataset']['m'] = None
    exp25['dataset']['s'] = None
    exp25['dataset']['s_list'] = None

    ## Category parameters
    ### In order, each element contains: k, d, pdi, pd
    exp25['dataset']['custom'] = None 
    
    # Simulation parameters
    exp25['sim']['layer_params'], exp25['sim']['sim_params'] = sim_config() 
    
    # Repetition parameter
    exp25['repeat'] = 0

    with open(save_fname, 'w') as f:
        json.dump(exp25, f, indent=3)
