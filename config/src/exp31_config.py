...
def sim_config():
    ...
    sim_params = OrderedDict({'train_ratio': 0.7, 'AE_epochs': 100, 
                              ...
                              'AE_thresh': 0.01, '_patience': 5, 'class_epochs': 100, 'class_batch_size': 8, 'class_lr': 10e-2, 
                              'class_wd': 10e-3, 'class_monitor': 'loss', 'class_thresh': 0.01, 'training': 'early_stop', 'eval_mode': 'batch',
                              'inplace_noise': True, 'rep_type': 'lin', 'save_model': True, 'metric':'cosine', 'verbose': False, 'rep_diff': True})
...
def main(**kwargs):
    ...
    exp31['dataset'] = {}
    exp31['sim'] = {}
    exp31['mode'] = 'benchmark'
    exp31['model'] = 'full-linear'
    exp31['exp_name'] = 'exp31'
    exp31['dataset_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'data', 'benchmark_stimuli', 'N8'))
    exp31['save_dir'] = osp.abspath(osp.join(Path(__file__).parent, '..', '..', 'results'))
    
    ## Macrofeature parameters
    exp31['dataset']['size'] = None
    exp31['dataset']['i'] = None
    exp31['dataset']['k'] = None
    exp31['dataset']['l'] = None
    exp31['dataset']['m'] = None
    exp31['dataset']['s'] = None
    exp31['dataset']['s_list'] = None

    ## Category parameters
    ### In order, each element contains: k, d, pdi, pd
    exp31['dataset']['custom'] = None 
    
    # Simulation parameters
    exp31['sim']['layer_params'], exp31['sim']['sim_params'] = sim_config() 
    
    # Repetition parameter
    exp31['repeat'] = 0

    with open(save_fname, 'w') as f:
        json.dump(exp31, f, indent=3)
